    ...
    return StreamBuilder(
        ...
        builder: (context, snapshot) {
          ...
          for (var event in events!) {
            // Add null checks to prevent NoSuchMethodError
            final String eventName = event.data()['Event Name'] ?? '';
            final String eventId = event.data()['ID']?.toString() ?? '';
            final String eventDate = event.data()['Date'] ?? '';
            final String generatedUser = event.data()['Generated User'] ?? '';

            if (eventName.toLowerCase().contains(widget.searchText.toLowerCase()) ||
                eventId.toLowerCase().contains(widget.searchText.toLowerCase()) ||
                eventDate.toLowerCase().contains(widget.searchText.toLowerCase()) ||
                generatedUser.toLowerCase().contains(widget.searchText.toLowerCase())) {
              if ((widget.selectedStatus == 'ALL') ||
                ...
                if ((widget.selectedUserType == 'ALL') ||
                    ((widget.selectedUserType == 'MY EVENTS') &&
                        (generatedUser == loggedInUser?.email)) ||
